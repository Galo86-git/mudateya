...
/* ── MODAL CONFIRMACIÓN ─────────────────────────────────── */
.confirm-overlay{position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.5);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:1rem;animation:fadeIn .15s ease}
.confirm-box{background:#fff;border-radius:16px;padding:1.8rem 1.6rem;max-width:380px;width:100%;box-shadow:0 24px 60px rgba(0,0,0,.25);animation:slideUp .15s ease}
.confirm-icon{font-size:2rem;text-align:center;margin-bottom:.8rem}
.confirm-title{font-family:'Bebas Neue',sans-serif;font-size:1.3rem;letter-spacing:1px;color:#0F1923;text-align:center;margin-bottom:.4rem}
.confirm-msg{font-size:13px;color:#475569;text-align:center;line-height:1.6;margin-bottom:1.4rem}
.confirm-btns{display:flex;gap:8px;justify-content:center}
.confirm-btn-cancel{padding:10px 22px;border-radius:10px;border:1px solid #CBD5E1;background:#fff;color:#475569;font-size:13px;font-weight:600;cursor:pointer;font-family:'Inter',sans-serif;transition:all .2s}
.confirm-btn-cancel:hover{border-color:#94A3B8;color:#0F1923}
.confirm-btn-ok{padding:10px 22px;border-radius:10px;border:none;font-size:13px;font-weight:700;cursor:pointer;font-family:'Inter',sans-serif;transition:all .2s}
.confirm-btn-ok.danger{background:#EF4444;color:#fff}.confirm-btn-ok.danger:hover{background:#DC2626}
.confirm-btn-ok.primary{background:#1A6FFF;color:#fff}.confirm-btn-ok.primary:hover{background:#0F52CC}
.confirm-btn-ok.success{background:#22C36A;color:#fff}.confirm-btn-ok.success:hover{background:#17A356}
...
async function accionTransfer(id, accion){
  var esConfirmar = accion === 'confirmar';
  confirmar({
    icon:    esConfirmar ? '💸' : '🚫',
    titulo:  esConfirmar ? '¿Confirmar transferencia?' : '¿Rechazar transferencia?',
    mensaje: esConfirmar
      ? 'Confirmá que ya realizaste la transferencia al mudancero. Esta acción queda registrada.'
      : '¿Seguro que querés rechazar esta transferencia? Podés volver a procesarla luego.',
    textoOk: esConfirmar ? '✓ Sí, confirmar' : '✗ Sí, rechazar',
    tipo:    esConfirmar ? 'success' : 'danger',
    onOk: function() { _doAccionTransfer(id, accion); }
  });
}
async function _doAccionTransfer(id, accion){
...
async function cambiarEstado(nuevoEstado){
  ...
  var nombre = m[1] || 'este mudancero';
  var esAprobar = nuevoEstado === 'Aprobado';
  confirmar({
    icon:     esAprobar ? '✅' : '🚫',
    titulo:   esAprobar ? '¿Aprobar a ' + nombre + '?' : '¿Rechazar a ' + nombre + '?',
    mensaje:  esAprobar
      ? 'El mudancero va a recibir un email de activación y podrá aceptar pedidos desde ya.'
      : 'El mudancero será notificado del rechazo. Podés revertirlo manualmente si es necesario.',
    textoOk:  esAprobar ? '✓ Sí, aprobar' : '✗ Sí, rechazar',
    tipo:     esAprobar ? 'success' : 'danger',
    onOk: function() { _doCambiarEstadoAdmin(nuevoEstado); }
  });
}
async function _doCambiarEstadoAdmin(nuevoEstado){
  if(currentMudanceroIdx === null) return;
  var m = mudancerosData[currentMudanceroIdx];
...
function confirmar(opciones) {
  ...
  overlay.innerHTML =
    '<div class="confirm-box">' +
      '<div class="confirm-icon">' + (opciones.icon || '⚠️') + '</div>' +
      '<div class="confirm-title">' + (opciones.titulo || '¿Confirmar acción?') + '</div>' +
      '<div class="confirm-msg">' + (opciones.mensaje || '¿Estás seguro?') + '</div>' +
      '<div class="confirm-btns">' +
        '<button class="confirm-btn-cancel" id="confirmCancelar">Cancelar</button>' +
        '<button class="confirm-btn-ok ' + (opciones.tipo || 'primary') + '" id="confirmOk">' + (opciones.textoOk || 'Confirmar') + '</button>' +
      '</div>' +
    '</div>';
  document.body.appendChild(overlay);
  document.getElementById('confirmCancelar').onclick = function() { overlay.remove(); };
  overlay.onclick = function(e) { if (e.target === overlay) overlay.remove(); };
  document.getElementById('confirmOk').onclick = function() {
    overlay.remove();
    if (opciones.onOk) opciones.onOk();
  };
}
</script>
</body>
